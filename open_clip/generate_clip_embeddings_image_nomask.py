#!/usr/bin/env python3
"""Generate image-only no-mask embeddings from a pretrained CLIP model (no CSN projection head)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Generate image-only no-mask CLIP embeddings (raw, no CSN head)")
    ...
    parser.add_argument("--output-prefix", type=str, default="clip_test")
...
def main() -> None:
    """Load CLIP, encode images in batches, and save raw (no-CSN) embeddings to disk."""
    args = parse_args()
    ...
    for i in tqdm(range(0, split_indices.size, args.batch_size), desc="Embedding batches"):
        ...
        with torch.no_grad():
            z_img = model.encode_image(x_img).float()
            if args.normalize:
    ...
    metadata = {
        ...
        "embed_dim": int(image_super.shape[1]),
        "image_only": True,
